...
namespace Wdem.LegacySource.Tests
{
  public class EngineTests
  {
    ...
    public async Task RunAsync_ShouldApplyRegistryTweaksAndSystemSettings_WhenConfigured()
    {
      ...
      {
        Path = "HKCU\\Software\\Wdem.LegacySource",
        Name = "SettingA",
        Value = 1,
        Type = "dword"
      });
      ...
      // Assert
      _mockRegistry.Verify(r => r.Apply(It.Is<RegistryTweak>(t => t.Path == "HKCU\\Software\\Wdem.LegacySource" && t.Name == "SettingA"), false), Times.Once);
      ...
      _mockStateService.Verify(s => s.MarkAsApplied(It.IsAny<string>()), Times.Never);
      _mockStateService.Verify(s => s.SaveState(It.Is<StateData>(state => state.AppliedItems.SetEquals(new[] { "reg:HKCU\\Software\\Wdem.LegacySource|SettingA", "reg:HKCU\\Software\\Preset|PresetSetting" }))), Times.Once);
    }
    ...
    public async Task RunAsync_ShouldManageServicesAndScheduledTasks_WhenConfigured()
    {
      ...
      config.ScheduledTasks.Add(new ScheduledTaskConfig { Name = "TestTask", Path = "\\\\Wdem.LegacySource\\\\Test", Description = "Task" });
      var mockLogger = new Mock<ILogger>();
      var engine = CreateEngine(mockLogger);
      ...
      _mockScheduledTaskService.Verify(t => t.Apply(It.Is<ScheduledTaskConfig>(task => task.Name == "TestTask" && task.Path == "\\\\Wdem.LegacySource\\\\Test"), false), Times.Once);
    }
    ...
    public async Task RunAsync_ShouldNotMarkFailedRegistryApplyAsApplied()
    {
      ...
      // Assert
      _mockStateService.Verify(s => s.MarkAsApplied("reg:HKCU\\Software\\Wdem.LegacySource|FailedSetting"), Times.Never);
      _mockStateService.Verify(s => s.SaveState(It.IsAny<StateData>()), Times.Never);
    }
