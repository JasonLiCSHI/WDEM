  ...
  public class StateWriterTests
  {
    ...
    public void RecordStep_ShouldWriteAndLoad()
    {
      ...
      try
      {
        ...
        var step = new Wdem.LegacySource.Models.StepResult
        {
          StepId = "winget:TestApp",
          StepType = "app",
          StepName = "TestApp",
          Status = Wdem.LegacySource.Models.StepStatus.Succeeded,
        ...
        Assert.Equal(Wdem.LegacySource.Models.StepStatus.Succeeded, loaded[step.StepId].Status);
      }
      finally
      {
        if (File.Exists(tmp)) File.Delete(tmp);
