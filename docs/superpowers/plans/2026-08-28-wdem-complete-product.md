...
`ReSharperSettingsProvider` registers `resharper-settings`/`file`, requires `resharper` as a dependency, atomically copies the verified `.DotSettings` source to `%APPDATA%\JetBrains\Shared\vAny\GlobalSettingsStorage.DotSettings`, records source/destination hashes, and verifies destination existence/hash without starting ReSharper. Resolve `%APPDATA%` through the current-user known-folder API and keep the profile path relative to the trusted JetBrains root; do not perform unrestricted environment-string expansion.

`VisualStudioSettingsProvider` registers `visual-studio-settings`/`visual-studio-settings`, requires `visual-studio`, verifies source hash, snapshots the source to a profile-specified `settingsStorePath` below `%LOCALAPPDATA%\Microsoft\VisualStudio\<major>.0_<instanceId>\Settings`, and invokes the selected instance with the official tokenized command `devenv.exe /ResetSettings <snapshotPath> /Command Exit` only during Apply. Detect/Verify never launch Visual Studio: they check the stored snapshot path, source hash, and target instance evidence. Plans bind the detected destination path/existence/hash and reject stale apply and no-op execution. After `devenv.exe` exits successfully, commit and final verification finish non-cancellably so late cancellation cannot discard completed evidence. If the profile declares a target settings-store hash and it differs, final apply fails with `ConfigurationMismatch`; if it declares incompatible edition/channel, block import with `ConfigurationError`.
