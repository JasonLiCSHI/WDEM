...
public static class ShellCompletionGenerator
{
  ...
  private static string GeneratePowerShell(RootCommand rootCommand)
  {
    ...
    sb.AppendLine("# Wdem.LegacySource CLI tab completion for PowerShell");
    ...
    var completions = CollectCompletions(rootCommand, Regex.Escape("Wdem.LegacySource"));
    ...
    scriptBlock.AppendLine("        ($commandLine -match $_.Context -or $_.Context -eq 'Wdem.LegacySource') -and");
  ...
  private static string GenerateBash(RootCommand rootCommand)
  {
    ...
    sb.AppendLine("# Wdem.LegacySource CLI tab completion for Bash");
