...
| State and inputs | Run `inspect-smoke.ps1`. Its first scenario unsets `WDEM_COMPANY_VSIX_PATH` and `WDEM_COMPANY_VSIX_SHA256` and leaves `company-vs-extension` unselected; its second selects the VSIX while routing both acquisition URLs to a local TCP trap. | State and redacted logs are below `%LOCALAPPDATA%\WDEM`; the retired `%LOCALAPPDATA%\WinHome` tree, secure artifact roots, and bootstrapper download directory are not written. Unresolved variables belonging only to the unselected optional VSIX do not fail Inspect, and neither acquisition source opens a connection. | Record both passing smoke scenarios and the `%LOCALAPPDATA%\WDEM` tree. The smoke reports and trap profile are temporary and removed. |
